...
#endif
/*--prize-factory reset restore-add --liuweiquan-20151010 start--*/
void copy_file(const char *src,const char *dstFilePath)
{
	...
	while ((read = fread(buffer, 1, buffer_size, fin)) > 0)
	{
		if (fwrite(buffer, 1, read, fout) != read)
		{
			printf("fwrite error \n");
	...
	if (fclose(fout) != 0)
	{
		printf("fclose error out \n");
		fclose(fin);
		return;
	}
	if (fclose(fin) != 0)
	{
		printf("fclose error in \n");
		return;
	}
}

void prize_backup2tmp(){
	ensure_path_mounted("/data");
	copy_file("/data/prize_backup/prize_factory_data","/tmp/prize_factory_data");
	ensure_path_unmounted("/data");
}

void prize_tmp2backup(){
	if (opendir("/data/prize_backup") == NULL) {
        printf("dir_name = '%s' does not exist, create it.\n", "/data/prize_backup");
        if (mkdir("/data/prize_backup", 0666)) {
            printf("can not create '%s' : %s\n", "/data/prize_backup");
        }
    }
	copy_file("/tmp/prize_factory_data","/data/prize_backup/prize_factory_data"); 
	chmod("/data/prize_backup/prize_factory_data", 0777);
    chown("/data/prize_backup/prize_factory_data", 1000, 1000);   // system user
	chmod("/data/prize_backup", 0777);
    chown("/data/prize_backup", 1000, 1000);   // system user
}
/*--prize-factory reset restore-add --liuweiquan-20151010 end--*/
int
